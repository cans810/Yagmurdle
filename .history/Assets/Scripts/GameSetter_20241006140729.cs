...
for (int i = 0; i < gridSize; i++)
{
    for (int j = 0; j < gridSize; j++)
    {
        ...
        // Adjust the position according to i (row) and j (column)
        letterObject.transform.position = new Vector3(
            gridStartPos.x + (j * letterXOffset),  // Use 'j' for x (column)
            gridStartPos.y - (i * letterYOffset),  // Use 'i' for y (row), and subtract to go down
            gridStartPos.z
        );

        letterObjects.Add(letterObject);

        // Correctly assign the point with (i, j) as (row, column)
        letterObject.GetComponent<Letter>().point = new Vector2(i, j);

        // Set the letter representation as the point for now (or modify as per your logic)
