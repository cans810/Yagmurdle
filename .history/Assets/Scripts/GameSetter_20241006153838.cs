...
public class GameSetter : MonoBehaviour
{
            ...
            if (child.tag == "Letter")
            {
                ...
                if (letter.currentlyActive && !string.IsNullOrEmpty(letter.inputField.text))
                {
                    if (j == gridSize - 1)
                    {
                        ...
                        {
                            Debug.LogWarning($"Answer for row {i} already exists. Overwriting the value.");
                        ...
                        foreach (Transform c in parentTransform)
                        {
                            if (c.tag == "Letter")
                            {
                                Letter letterComponent = c.GetComponent<Letter>();

                                if (letterComponent.point.x == i)
                                {
                                    // Get the letter character from the input field
                                    char givenLetter = letterComponent.inputField.text.Length > 0 ? letterComponent.inputField.text[0] : '\0';

                                    // Check if the letter is valid (not empty)
                                    if (givenLetter != '\0')
                                    {
                                        // Check if the letter is correct and in the right position
                                        if (ContainsAndCorrectSpot(randomWord, givenLetter, (int)letterComponent.point.y))
                                        {
                                            letterComponent.SetColor(Color.green); // Correct letter and position
                                        }
                                        // Check if the letter is in the word but in the wrong position
                                        else if (ContainsAndCorrectSpot(randomWord, givenLetter, (int)letterComponent.point.y) == false)
                                        {
                                            letterComponent.SetColor(Color.yellow); // Letter is in the word but in the wrong position
                                        }
                                        // If the letter is not in the word, do nothing (color stays the same)
                                    }
                                }
                            }
                        }
    ...
    public bool ContainsAndCorrectSpot(string realAnswer, char givenLetter, int index)
    {
        ...
        // Letter is not in the correct position, check if it's in the word
        for (int j = 0; j < realAnswer.Length; j++)
        {
            if (givenLetter == realAnswer[j])
            {
                Debug.Log($"Letter '{givenLetter}' is in the word but in the wrong position.");
                return false; // Letter is found but in the wrong position
            }
        }

        Debug.Log($"Letter '{givenLetter}' is not in the word.");
        return false; // Letter is not found in the word
    }
