...
public class GameSetter : MonoBehaviour
{
    ...
    void Update()
    {
        if (!gameGenerated){
            ...
            for (int i = 0; i < gridSize; i++)
            {
                for (int j = 0; j < gridSize; j++)
                {
                    ...
                    letterObject.GetComponent<Letter>().rowBelongedTo = j;
        ...
        if (gameGenerated){
            ...
            foreach (Transform child in parentTransform)
            {
                if (child.tag == "Letter" && child.gameObject.GetComponent<Letter>().rowBelongedTo == currentRowFilling){
                    if (!child.GetComponent<Letter>().letterText.Equals("") || child.GetComponent<Letter>().letterText != null){
                        child.GetComponent<Letter>().filled = true;
                    }
                }
            }
