...
public class GameSetter : MonoBehaviour
{
    ...
    void CheckGuess()
    {
        ...
        if (playerGuess == targetWord)
        {
            ...
            // Trigger win condition (e.g., end the game, reset the grid)
        }
        else if (currentGuess >= maxGuesses)
        {
            Debug.Log("Player ran out of guesses!");
            // Trigger loss condition (e.g., end the game, reveal the word)
        }
        else
        {
            currentRowFilling = 0;
            currentRowObjects = GetCurrentRowObjects();  // Move to the next row
        }
    }

    void HandleLetterInput()
    {
        // Only allow letter input when the current row is not filled
